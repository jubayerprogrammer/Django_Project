...
# def send_reset_password_email(recipient_email,reset_url):
    ...
    # from_email = "no_reply@domailtrap.com"
    # to_email = [recipient_email]

    # html_content = render_to_string("account/reset_password_email.html",{"reset_url":reset_url})

    # # create the email body with both HTML and Plain next version
    # text_content = strip_tags(html_content)
    # email = EmailMultiAlternatives(subject,text_content,from_email,to_email)
    # email.attach_alternative(html_content,"text/html").SendEmailThred(email).start()
